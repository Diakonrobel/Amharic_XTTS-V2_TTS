...
def main():
    ...
    parser.add_argument("--input_audio", type=str, required=False, help="Path to the single input audio file (MP3, WAV, FLAC, etc.)")
    ...
    parser.add_argument("--lang", type=str, default="en", help="Language of the dataset (ISO 639-1 code, e.g., en, es, fr). Use 'am' or 'amh' for Amharic.", choices=["en","es","fr","de","it","pt","pl","tr","ru","nl","cs","ar","zh","hu","ko","ja","am","amh"])
    ...
    # Advanced Dataset Processing Arguments
    parser.add_argument("--srt_file", type=str, default=None, help="Path to SRT subtitle file (for SRT + media processing)")
    parser.add_argument("--media_file", type=str, default=None, help="Path to media file (audio/video) corresponding to SRT file")
    parser.add_argument("--youtube_url", type=str, default=None, help="YouTube video URL to download and process")
    parser.add_argument("--youtube_transcript_lang", type=str, default="en", help="Preferred language for YouTube transcript extraction")
    parser.add_argument("--slice_audio", action="store_true", help="Enable RMS-based audio slicing for input audio")
    parser.add_argument("--slicer_threshold_db", type=float, default=-40.0, help="Silence threshold in dB for audio slicing")
    parser.add_argument("--slicer_min_length", type=float, default=5.0, help="Minimum segment length in seconds for audio slicing")
    parser.add_argument("--slicer_min_interval", type=float, default=0.3, help="Minimum silence interval in seconds for audio slicing")
    parser.add_argument("--slicer_max_sil_kept", type=float, default=0.5, help="Maximum silence kept at segment boundaries in seconds")
    parser.add_argument("--use_g2p", action="store_true", help="Enable Amharic G2P preprocessing (for Amharic language)")
    parser.add_argument("--g2p_backend", type=str, default="epitran", choices=["epitran", "transphone", "rule_based"], help="G2P backend to use for Amharic (epitran recommended)")
...
args = parser.parse_args()
    ...
    input_modes = sum([
        ...
        bool(args.srt_file and args.media_file),
        bool(args.youtube_url)
    ])
    
    if input_modes == 0:
        print("Error: No input method specified!")
        print("Please provide one of:")
        print("  --input_audio <file>")
        print("  --srt_file <file> --media_file <file>")
        print("  --youtube_url <url>")
        sys.exit(1)
    
    if input_modes > 1:
        print("Error: Multiple input methods specified! Please use only one.")
        sys.exit(1)

    # --- Setup Paths ---
    output_dir_base = Path(args.output_dir_base)
    
    # Determine output name based on input method
    if args.model_name:
        output_name = args.model_name
    elif args.input_audio:
        output_name = Path(args.input_audio).stem
    elif args.srt_file:
        output_name = Path(args.srt_file).stem
    elif args.youtube_url:
        # Extract video ID or use generic name
        import re
        video_id_match = re.search(r'(?:v=|/)([0-9A-Za-z_-]{11}).*', args.youtube_url)
        output_name = video_id_match.group(1) if video_id_match else "youtube_model"
    else:
        output_name = "xtts_model"
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # --- Step 0: Advanced Dataset Processing ---
        if args.srt_file and args.media_file:
            print("\n=== Mode: SRT + Media File Processing ===")
            print(f"SRT File: {args.srt_file}")
            print(f"Media File: {args.media_file}")
            
            try:
                train_csv, eval_csv, duration = srt_processor.process_srt_with_media(
                    srt_path=args.srt_file,
                    media_path=args.media_file,
                    output_dir=str(output_dir_dataset),
                    language=args.lang
                )
                train_csv_path = Path(train_csv).resolve()
                eval_csv_path = Path(eval_csv).resolve()
                print(f"✓ SRT processing complete! Processed {duration:.1f}s of audio")
            except Exception as e:
                print(f"Error processing SRT + media: {e}")
                traceback.print_exc()
                sys.exit(1)
                
        elif args.youtube_url:
            print("\n=== Mode: YouTube Video Download ===")
            print(f"YouTube URL: {args.youtube_url}")
            print(f"Transcript Language: {args.youtube_transcript_lang}")
            ...
            try:
                ...
                audio_path, srt_path, info = youtube_downloader.download_youtube_video(
                    ...
                    output_dir=temp_dir,
                    language=args.youtube_transcript_lang,
                    audio_only=True,
                ...
                if not srt_path:
                    print("Error: No transcripts/subtitles available for this video")
                    sys.exit(1)
                
                print(f"✓ Downloaded: {info.get('title', 'Unknown')}")
                print(f"  Duration: {info.get('duration', 0):.0f}s")
                ...
                print(f"Error downloading/processing YouTube: {e}")
                traceback.print_exc()
                sys.exit(1)
                
        elif args.input_audio:
            print("\n=== Mode: Direct Audio Input ===")
            ...
            # Check if audio slicing is enabled
            if args.slice_audio:
                print("\n--- RMS-Based Audio Slicing Enabled ---")
                print(f"Parameters:")
                print(f"  Threshold: {args.slicer_threshold_db} dB")
                print(f"  Min Length: {args.slicer_min_length}s")
                print(f"  Min Interval: {args.slicer_min_interval}s")
                print(f"  Max Silence Kept: {args.slicer_max_sil_kept}s")
                
                try:
                    # Load audio
                    import librosa
                    import soundfile as sf
                    audio, sr = librosa.load(str(input_audio_path), sr=22050, mono=True)
                    
                    # Initialize slicer
                    slicer = audio_slicer.Slicer(
                        sr=sr,
                        threshold=args.slicer_threshold_db,
                        min_length=int(args.slicer_min_length * 1000),
                        min_interval=int(args.slicer_min_interval * 1000),
                        hop_size=10,
                        max_sil_kept=int(args.slicer_max_sil_kept * 1000)
                    )
                    ...
                    # Save segments
                    wavs_dir = output_dir_dataset / "wavs"
                    wavs_dir.mkdir(parents=True, exist_ok=True)
                    
                    segment_files = []
                    for i, chunk in enumerate(chunks):
                        segment_filename = f"{input_audio_path.stem}_segment_{str(i).zfill(4)}.wav"
                        segment_path = wavs_dir / segment_filename
                        sf.write(str(segment_path), chunk, sr)
                        segment_files.append(str(segment_path))
                    
                    print(f"✓ Segments saved, now transcribing with Whisper...")
                    
                    # Use standard preprocessing with Whisper
                    device = "cuda" if torch.cuda.is_available() else "cpu"
                    compute_type = "float16" if torch.cuda.is_available() else "float32"
                    ...
                    train_csv_path = Path(train_meta).resolve()
                    eval_csv_path = Path(eval_meta).resolve()
                    print(f"✓ Audio slicing and transcription complete!")
                except Exception as e:
                    print(f"Error during audio slicing: {e}")
                    traceback.print_exc()
                    sys.exit(1)
            else:
                # Standard audio preparation (convert and trim if needed)
                ...
                # --- Step 1: Data Processing (Whisper) ---
                status, train_csv, eval_csv = preprocess_dataset_headless(
                    audio_file_path=prepared_audio_path,
                    language=args.lang,
                    whisper_model_name=args.whisper_model,
                    dataset_out_path=str(output_dir_dataset.resolve())
                )
        ...
        if not train_csv_path or not eval_csv_path:
            print("Error: Dataset processing did not produce train/eval CSV files!")
            sys.exit(1)
