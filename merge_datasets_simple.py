#!/usr/bin/env python3
"""
Dataset Merger for XTTS Fine-tuning (Automatic Mode - No Dependencies)
...
AUTO_CLEANUP = "move"  # Options: "keep", "delete", "move"
...
def merge_csv_files(main_csv, temp_csvs, output_csv):
    ...
    if main_csv.exists():
        ...
        if main_rows:
            header = main_rows[0]
            all_rows.extend(main_rows[1:])
            print_info(f"Main CSV has {len(main_rows)-1} rows")
    else:
    ...
    for temp_csv in temp_csvs:
        if temp_csv.exists():
            temp_rows = read_csv_rows(temp_csv)
            if temp_rows:
                if header is None:
                    header = temp_rows[0]
                all_rows.extend(temp_rows[1:])
                print_info(f"  {temp_csv.parent.name}: {len(temp_rows)-1} rows")
                total_temp_rows += len(temp_rows) - 1
        else:
            print_warning(f"  {temp_csv} not found, skipping")
    
    # Remove duplicates based on first column (audio_file)
    seen = set()
    unique_rows = []
    duplicates = 0
    ...
    for row in all_rows:
        if row and len(row) > 0:
            key = row[0]  # First column is usually audio_file
            if key not in seen:
                seen.add(key)
    ...
    # Write merged file
    if header:
        write_csv_rows(output_csv, [header] + unique_rows)
        print_success(f"Merged CSV saved: {len(unique_rows)} total rows")
        return len(unique_rows), total_temp_rows
    else:
...
def main():
    print_header("Dataset Merger - AUTOMATIC MODE")
    ...
    if invalid_count > 0:
        print_warning(f"{invalid_count} datasets have missing files - continuing anyway")
    
    # Auto-confirm
    print_warning(f"\n🤖 AUTO-MODE: Merging {len(temp_dirs)} temp datasets into main dataset")
    ...
    # Automatic cleanup
    print_header("Automatic Cleanup")
    
    if AUTO_CLEANUP == "move":
    ...
    elif AUTO_CLEANUP == "delete":
        print_warning("Deleting temp datasets...")
        for temp_dir in temp_dirs:
            shutil.rmtree(temp_dir)
            print_success(f"  Deleted: {temp_dir.name}")
    
    else:
        print_info("Keeping temp datasets (no cleanup)")
